...
export function useHeaderCollapse(
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      //
      // The baseline is capped to the range, because the list can get shorter
      // under the reader: a session deleted, a filter applied. The browser
      // pulls `scrollTop` back to the new end, and that is not the reader
      // moving, so it must not be read as a turn upward.
